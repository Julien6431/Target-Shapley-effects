    ...
    **kwargs : aux_distr : OPENTURNS DISTRIBUTION
    ...
    if withIS:
        if len(kwargs) == 1:
            ...
            E_2Y = estimated_mean**2 - (np.mean((Y * w) ** 2) - estimated_mean**2) / (
                Nv - 1
            )
            estimated_var = estimated_mean - E_2Y

            if type_estimator == "dMC":
                get_closed_sobol = lambda u: (
                    estimated_mean
                    - compute_closed_sobol_MC_IS(
                        u, Nu, Ni, phi, t, input_distr, mean_g, cov_g
                    )
                )
            elif type_estimator == "PF":
                get_closed_sobol = lambda u: (
                    compute_closed_sobol_PF_IS(
                        u, Nu, phi, t, input_distr, mean_g, cov_g
                    )
                    - E_2Y
                )
        ...
        if type_estimator == "dMC":
            get_closed_sobol = lambda u: compute_closed_sobol_MC(
                u, Nu, Ni, phi, t, mean_f, cov_f
            )
        elif type_estimator == "PF":
            get_closed_sobol = lambda u: (
                compute_closed_sobol_PF(u, Nu, phi, t, mean_f, cov_f)
                - estimated_mean**2
            )
            ...
            if len(u) == 0:
                ...
                for i in range(len(u)):
                    shapley[u[i]] += (
                        1
                        / binom(dim - 1, len(u) - 1)
                        * (closed_Sobol[u] - closed_Sobol[tuple(np.delete(u, i))])
                    )
            else:
                u_arr = np.array(u)
    ...
    **kwargs : init_distr : OPENTURNS DISTRIBUTION
    ...
    if withIS:
        ...
        if type_estimator == "dMC":
            get_closed_sobol = lambda u: (
                estimated_mean
                - compute_closed_sobol_MC_IS_gd(
                    u, Nu, Ni, X, Y, w, init_distr, aux_distr, Z
                )
            )
        elif type_estimator == "PF":
            get_closed_sobol = lambda u: (
                compute_closed_sobol_PF_IS_gd(u, Nu, X, Y, w, init_distr, aux_distr, Z)
                - E_2Y
            )
        ...
        if type_estimator == "dMC":
            ...
            get_closed_sobol = lambda u: (
                compute_closed_sobol_PF_gd(u, Nu, Z, Y) - estimated_mean**2
            )
