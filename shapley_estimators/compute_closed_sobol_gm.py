...
def compute_closed_sobol_MC(u, Nu, Ni, phi, t, mean_f, cov_f):
    ...
    sample_No = np.random.multivariate_normal(
        mean_f[com_u], cov_matrix_com_P_pi_j, size=Nu
    )
    cond_cov_matrix = (
        cov_matrix_P_pi_j
        - cross_cov_matrix @ inv_cov_matrix_com_P_pi_j @ cross_cov_matrix.T
    )

    # Double Monte Carlo loop
    samples = np.zeros((Nu, dim, Ni))
    for l in range(Nu):
        cond_mean = mean_f[u] + cross_cov_matrix @ inv_cov_matrix_com_P_pi_j @ (
            sample_No[l] - mean_f[com_u]
        )
...
def compute_closed_sobol_PF(u, Nu, phi, t, mean_f, cov_f):
    ...
    cond_cov_matrix = (
        cov_matrix_com_P_pi_j
        - cross_cov_matrix @ inv_cov_matrix_P_pi_j @ cross_cov_matrix.T
    )
    ...
    # Pick-Freeze loop
    for l in range(Nu):
        cond_mean = mean_f[com_u] + cross_cov_matrix @ inv_cov_matrix_P_pi_j @ (
            sample_No[l] - mean_f[u]
        )
...
def compute_closed_sobol_MC_IS(u, Nu, Ni, phi, t, input_distr, mean_g, cov_g):
    ...
    new_cov_g = (
        esti_cov_matrix_g
        - cross_cov_matrix_g @ inv_cond_cov_matrix_g @ cross_cov_matrix_g.T
    )
    ...
    for l in range(Nu):
        new_mean_g = mean_g[u] + cross_cov_matrix_g @ inv_cond_cov_matrix_g @ (
            sample_No[l] - mean_g[com_u]
        )
    ...
    biais = np.mean(
        (np.mean(outputs**2, axis=1) - np.mean(outputs, axis=1) ** 2) / w_minus_u
    ) / (Ni - 1)
...
def compute_closed_sobol_PF_IS(u, Nu, phi, t, input_distr, mean_g, cov_g):
    ...
    # Pick-Freeze loop
    for l in range(Nu):
        new_mean_g = mean_g[com_u] + cross_cov_matrix_g @ inv_cond_cov_matrix_g @ (
            sample_No[l] - mean_g[u]
        )
