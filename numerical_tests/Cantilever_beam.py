...
input_distr = ot.ComposedDistribution(
    [distr_FX, distr_FY, distr_E, distr_w, distr_t, distr_L], Copule_normale
)
...
for j in tqdm(range(n_rep)):
    ...
    shapleys_mc_box[j, :], _ = rosa_shapley_effects_gd(
        Xf, Yf, Nu=No, Ni=3, aggregation="subset", type_estimator="dMC"
    )
    shapleys_pf_box[j, :], _ = rosa_shapley_effects_gd(
        Xf, Yf, Nu=No, aggregation="subset", type_estimator="PF"
    )

    Xg_SG = np.array(aux_distr_SG.getSample(N))
    Yg_SG = np.apply_along_axis(lambda x: phi(x) > t, 1, Xg_SG)
    shapleys_mc_isSG_box[j, :], _ = rosa_shapley_effects_gd(
        Xg_SG,
        Yg_SG,
        Nu=No,
        Ni=3,
        ...
        aux_distr=aux_distr_SG,
    )
    shapleys_pf_isSG_box[j, :], _ = rosa_shapley_effects_gd(
        Xg_SG,
        Yg_SG,
        Nu=No,
        aggregation="subset",
    ...
    shapleys_mc_isGM_box[j, :], _ = rosa_shapley_effects_gd(
        Xg_SG,
        Yg_SG,
        Nu=No,
        Ni=3,
        ...
        type_estimator="dMC",
        init_distr=input_distr,
        aux_distr=aux_distr_GM,
    )
    shapleys_pf_isGM_box[j, :], _ = rosa_shapley_effects_gd(
        Xg_SG,
        Yg_SG,
        Nu=No,
        aggregation="subset",
...
np.savez(
    "data/Cantilever_beam_boxplots.npz",
...
for j in tqdm(range(n_rep)):
    Xf = np.array(input_distr.getSample(N))
    Yf = np.apply_along_axis(lambda x: phi(x) > t, 1, Xf)
    shapleys_mc_box_norm[j, :], _ = rosa_shapley_effects_gd(
        Xf,
        Yf,
        Nu=No,
        Ni=3,
    ...
    shapleys_pf_box_norm[j, :], _ = rosa_shapley_effects_gd(
        Xf, Yf, Nu=No, aggregation="subset", type_estimator="PF", standardisation=True
    )

    Xg_SG = np.array(aux_distr_SG.getSample(N))
    Yg_SG = np.apply_along_axis(lambda x: phi(x) > t, 1, Xg_SG)
    shapleys_mc_isSG_box_norm[j, :], _ = rosa_shapley_effects_gd(
        Xg_SG,
        Yg_SG,
        Nu=No,
        Ni=3,
    ...
    shapleys_pf_isSG_box_norm[j, :], _ = rosa_shapley_effects_gd(
        Xg_SG,
        Yg_SG,
        Nu=No,
        aggregation="subset",
    ...
    Xg_GM = np.array(aux_distr_GM.getSample(N))
    Yg_GM = np.apply_along_axis(lambda x: phi(x) > t, 1, Xg_GM)
    shapleys_mc_isGM_box_norm[j, :], _ = rosa_shapley_effects_gd(
        Xg_SG,
        Yg_SG,
        Nu=No,
        Ni=3,
    ...
    shapleys_pf_isGM_box_norm[j, :], _ = rosa_shapley_effects_gd(
        Xg_SG,
        Yg_SG,
        Nu=No,
        aggregation="subset",
...
np.savez(
    "data/Cantilever_beam_boxplots_std.npz",
    theo_values=reference_values["ref"],
...
for j in range(n_rep):
    shapleys_mc_isSG_box_gm[j, :], _ = rosa_shapley_effects(
        phi,
        t,
        input_distr,
        Nv=Nv,
    ...
    shapleys_pf_isSG_box_gm[j, :], _ = rosa_shapley_effects(
        phi,
        t,
        input_distr,
        Nv=Nv,
...
np.savez(
    "data/Cantilever_beam_boxplots_gm.npz",
    theo_values=reference_values["ref"],
