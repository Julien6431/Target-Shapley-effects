        ...
        for u in subsets:
            if len(u) == 0:
                ...
                if (beta[com_u] == 0).all() == True:
                    ...
                    cond_cov_matrix = (
                        cov_matrix_com_u
                        - cross_cov_matrix @ inv_cov_matrix_u @ cross_cov_matrix.T
                    )
                    cond_mean = (
                        mean_f[com_u]
                        + (
                            cross_cov_matrix
                            @ inv_cov_matrix_u
                            @ ((X[:, u_arr] - mean_f[u_arr]).T)
                        ).T
                    )
                    Y = Phi(
                        (
                            t
                            - np.sum(beta[u_arr] * X[:, u_arr], axis=1)
                            - np.sum(beta[com_u] * cond_mean, axis=1)
                        )
                        / np.sqrt(beta[com_u].T @ cond_cov_matrix @ beta[com_u])
                    )
