...
beta = np.ones(3)
mean_f = np.zeros(3)
cov_f = np.array([[1, 0, 0], [0, 1, rho], [0, rho, 1]])
t = 4
input_distr = ot.Normal(ot.Point(dim), ot.CovarianceMatrix(cov_f))

theoretical_shapleys_3, stds_3 = compute_theoretical_values(
    t, beta, mean_f, cov_f, N_sample=10**6, n_rep=20
)
...
for j in tqdm(range(n_rep)):
    ...
    shapleys_mc_gd_box[j, :], _ = rosa_shapley_effects_gd(
        Xf,
        Yf,
        Nu=No,
        Ni=3,
    ...
    shapleys_pf_gd_box[j, :], _ = rosa_shapley_effects_gd(
    ...
    shapleys_mc_is_gd_box[j, :], _ = rosa_shapley_effects_gd(
        Xg,
        Yg,
        Nu=No,
        Ni=3,
    ...
    shapleys_pf_is_gd_box[j, :], _ = rosa_shapley_effects_gd(
        Xg,
        Yg,
        Nu=No,
        aggregation="subset",
...
for j in tqdm(range(n_rep)):
    shapleys_mc_box[j, :], _ = rosa_shapley_effects(
        phi,
        t,
        input_distr,
        Nv=Nv,
    ...
    shapleys_pf_box[j, :], _ = rosa_shapley_effects(
        phi, t, input_distr, Nv=Nv, Nu=No_pf, aggregation="subset", type_estimator="PF"
    )

    shapleys_mc_is_box[j, :], _ = rosa_shapley_effects(
        phi,
        t,
        input_distr,
        Nv=Nv,
    ...
    shapleys_pf_is_box[j, :], _ = rosa_shapley_effects(
        phi,
        t,
        input_distr,
        Nv=Nv,
...
np.savez(
    "data/3d_Gaussian_linear_boxplots.npz",
